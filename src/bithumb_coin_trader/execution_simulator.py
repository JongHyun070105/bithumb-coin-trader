...
class MarketOrderRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.requested_amount_krw is None and self.requested_quantity_btc is None:
            raise ValueError("Must specify either requested_amount_krw or requested_quantity_btc.")
...
class DeterministicTakerSimulator:
    ...
    def execute_order(
        ...
        """Executes a MarketOrderRequest against a single OrderBookSnapshot.
        ...
        """
        ref_book = order_time_book if order_time_book is not None else orderbook
        ...
        top_fill = orderbook.best_ask if side_norm == "BUY" else orderbook.best_bid
        mid_fill = orderbook.mid_price
        ...
        levels = orderbook.asks if side_norm == "BUY" else orderbook.bids
        if not levels:
            return ExecutionResult(
                request=request,
                order_timestamp=request.timestamp,
                fill_timestamp=orderbook.timestamp,
                status="REJECTED",
                side=side_norm,
                filled_quantity=0.0,
        ...
        if request.requested_amount_krw is not None:
            # Quote currency budget mode (e.g. buy 10,000,000 KRW worth of BTC)
            remaining_krw = request.requested_amount_krw

            for idx, (p, s) in enumerate(levels):
                level_max_krw = p * s
                if remaining_krw <= level_max_krw:
                    take_qty = remaining_krw / p
                    fills.append(ExecutionFill(level_index=idx, price=p, size=take_qty, notional_krw=remaining_krw))
                    total_filled_qty += take_qty
                    total_filled_krw += remaining_krw
                    remaining_krw = 0.0
                    break
                else:
                    fills.append(ExecutionFill(level_index=idx, price=p, size=s, notional_krw=level_max_krw))
                    total_filled_qty += s
                    total_filled_krw += level_max_krw
            ...
            # Base currency quantity mode (e.g. buy 0.5 BTC)
            assert request.requested_quantity_btc is not None
            remaining_qty = request.requested_quantity_btc

            for idx, (p, s) in enumerate(levels):
                if remaining_qty <= s:
                    take_krw = remaining_qty * p
                    fills.append(ExecutionFill(level_index=idx, price=p, size=remaining_qty, notional_krw=take_krw))
                    total_filled_qty += remaining_qty
                    total_filled_krw += take_krw
                    remaining_qty = 0.0
                    break
                else:
                    take_krw = s * p
                    fills.append(ExecutionFill(level_index=idx, price=p, size=s, notional_krw=take_krw))
                    total_filled_qty += s
                    total_filled_krw += take_krw
                    remaining_qty -= s
        ...
        # Handle complete exhaustion without full fill
        is_partial = (unfilled_krw > 1e-6 if request.requested_amount_krw is not None else unfilled_qty > 1e-9)

        if is_partial and not request.allow_partial:
            return ExecutionResult(
                request=request,
                order_timestamp=request.timestamp,
                fill_timestamp=orderbook.timestamp,
        ...
        if total_filled_qty <= 0.0 or total_filled_krw <= 0.0:
            return ExecutionResult(
                request=request,
                order_timestamp=request.timestamp,
                fill_timestamp=orderbook.timestamp,
        ...
        # BUY: higher vwap is adverse (positive slippage)
        # SELL: lower vwap is adverse (positive slippage)
        if side_norm == "BUY":
    ...
    def execute_with_latency(
        cls,
        request: MarketOrderRequest,
        orderbook_stream: Sequence[OrderBookSnapshot],
    ) -> ExecutionResult:
        ...
        if order_time_book is None:
            order_time_book = orderbook_stream[0]
        ...
        if fill_book is None:
            # Reached end of stream before latency expired
            fill_book = orderbook_stream[-1]
