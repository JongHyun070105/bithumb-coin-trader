...
## Redundancy design decision to resolve before validation

Evaluate whether the strict continuity contract requires overlapping Bithumb public WebSockets or a different independently sourced feed. Sharding stream types reduces one-socket blast radius but cannot mask a common endpoint outage. Overlap requires documented exchange permission and rate controls, production-like *local* load/fault tests, deterministic receive-time deduplication keyed by exchange/stream/market plus source identity/sequence or raw hash, and explicit provenance of which socket supplied each event. Bithumb's [basic WebSocket reference](https://apidocs.bithumb.com/reference/%EA%B8%B0%EB%B3%B8-%EC%A0%95%EB%B3%B4) limits connection attempts to 10 per IP per second and warns of 429/temporary blocks; the [connection-management reference](https://apidocs.bithumb.com/reference/%EC%97%B0%EA%B2%B0-%EA%B4%80%EB%A6%AC) describes ping/pong and idle behavior. Do not deploy redundancy simply by adding sockets without these checks. If overlap is declined, explicitly accept that a single remote/server/path outage may invalidate a strict qualifying hour.
