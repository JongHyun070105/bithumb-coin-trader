...
- Exact copy: persist the canonical raw event once, including its physical connection ID; increment `redundant_frames_received` and `deduplicated_frames`; append a compact provenance record containing both connection IDs and both hashes.
- Same nominal identity with different canonical payload hash: keep the first canonical event, quarantine the conflicting copy with both hashes and sources, increment `conflicting_duplicate_frames`, and fail coverage through writer health. No payload is silently selected as scientifically valid.
