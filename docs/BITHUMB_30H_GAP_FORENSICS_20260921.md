...
The PR #12 scheduler's finalized-FAIL skip now requires both epoch and run identity. A legacy finalized `FAIL` receipt without embedded identity is skippable only when a separate immutable archive-failure sidecar binds the canonical parsed receipt content, epoch, run and cohort by checksum. Conflicting aliases, corrupt receipts, wrong-cohort receipts, and unbound legacy failures remain visible but unskippable. The same identity rule applies to partial/ineligible completion, full-scan PASS, coverage receipts and raw receipts. The valid historical `FAIL` receipt stays visibly `FAIL` and is never rewritten; later cohorts can be discovered only through identity-bound evidence. Existing `SKIPPED_NON_QUALIFYING`, `INELIGIBLE_PARTIAL`, and full-scan status semantics were preserved and covered by the broader scheduler suite. Review comments on naive launch timestamps and heartbeat pruning were fixed in local source/tests. The two comments on sealed 90-minute historical wrappers remain as review findings because changing those artifacts would invalidate historical seals; future wrapper generation should include the witness and data-root arguments.
...
One physical socket cannot provide a zero-gap guarantee when the peer or path stops delivering frames. Splitting stream types over multiple sockets would reduce blast radius for a socket-specific failure, but a server-wide or common-route outage could still stop all of them. This follow-up branch implements two active-active public sockets with bounded first-arrival deduplication, conflict quarantine, per-source evidence and logical coverage formed from the union of confirmed physical-source intervals. Connection attempts are serialized at 250 ms, below the documented 10 requests per IP per second. The public documentation does not state a simultaneous public-connection cap, per-connection subscription cap or duplicate-subscription rule; exactly two connections is a bounded design choice and does not infer unlimited permission. A common endpoint, route or exchange outage can still stop both sources. The implementation and alternatives are recorded in `docs/BITHUMB_REDUNDANCY_DESIGN_20260921.md`.

## Verification and residual limits

The local fault suite covers normal pong, pong timeout with fresh frames, combined frame/pong loss, heartbeat interruption of blocked receive, stale timeout, near-simultaneous first-wins decisions, hanging close, close metadata, abrupt socket error, failed first connect then success, partial feed confirmation, other-exchange controls, event-loop lag, queue pressure, cohort boundary evidence, redundant-source failover, duplicate/conflict handling, bounded cache behavior, cancellation rollback, corrupt/foreign/wrong-cohort FAIL receipts, and six modeled server-outage durations. The replay fixture verifies the four gap measures from immutable source values and separately labels the synthetic redundant-source counterfactual. No test uses live Bithumb, AWS launch, order submission, or `test-results/` output.
...
Final verification counts for the follow-up candidate are recorded in `docs/FRESH_6H_NEXT_GO_NO_GO.md` after the candidate commit is frozen. These are local software checks, not a new validation run.
