...
def build_epoch_manifest(
    ...
    runtime_fingerprint = contract_data.get("runtime_config_fingerprint") or "unknown"
    ...
    for mf in manifest_files:
        ...
        if raw_p.exists() and not raw_sha:
            raw_sha, byte_count, rec_count = _stream_file_sha256(raw_p)
    ...
    default_hour = expected_hour_cohorts[0] if expected_hour_cohorts else "unknown"
    ...
    cohorts_to_check = expected_hour_cohorts or sorted(found_feeds_by_cohort.keys())
    for ch in cohorts_to_check:
        ...
        # Check receipt
        if len(cohorts_to_check) > 1 or contract_data.get("require_receipts", False):
    ...
    if contract_data.get("require_fullscan", False) or duration_seconds >= 259200:
    ...
    manifest_dict: dict[str, Any] = {
        ...
        "expected_hour_cohorts": expected_hour_cohorts,
...
def main() -> int:
    ...
    if args.strict and not res.get("sealed_complete"):
        print(f"ERROR: Epoch is incomplete: {res.get('missing_items')}")
        return 2
    return 0
