...
worktree="/var/lib/bitcoin-trader/runtime-worktrees/aws-validation-30h-20260915-v4"
python="/var/lib/bitcoin-trader/venv-pre-soak/bin/python"

sup_cmd_json=$("$python" -c '
import json
with open("/var/lib/bitcoin-trader/launch-artifacts/aws-validation-30h-20260915-v4/aws-validation-30h-20260915-v4.launch-command.json") as f:
    d = json.load(f)
...
exec "$python" "$worktree/scripts/launch_short_smoke_transient.py" \
  ...
  --qualification-schedule-path "/var/lib/bitcoin-trader/30h-validation/aws-validation-30h-20260915-v4/qualification_schedule.json" \
