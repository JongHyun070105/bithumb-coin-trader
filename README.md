...
- `cross_market_collector.py`, `microstructure_storage.py`: 빗썸/바이낸스/업비트 3개 거래소 실시간 체결·호가·시세 Append-Only 수집기 및 SHA-256 파티션 매니페스트 생성기. 완전성은 durable counter와 epoch별 감사를 통해 별도로 판정한다. (Strategy V9.1 데이터 인프라)
