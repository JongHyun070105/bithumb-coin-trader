...
artifacts_dir="/var/lib/bitcoin-trader/launch-artifacts/aws-validation-observability-6h-20260918-20260918T175000Z-v2"
...
if [ "$is_launch" = true ]; then
  ...
  planned_start="2026-09-18T17:50:00+00:00"
  qual_start="2026-09-18T18:00:00+00:00"
  ...
  obs_unit="bitcoin-trader-obs-aws-validation-observability-6h-run-20260918T175000Z-v2.service"
  echo "[LAUNCH] Step B: Starting runtime observer unit $obs_unit (pinned to user: bitcoin-trader)..."
  systemd-run \
    --unit="$obs_unit" \
    --description="Runtime Observer for aws-validation-observability-6h-run-20260918T175000Z-v2" \
    --service-type=simple \
    --no-block \
    --uid=bitcoin-trader \
    --property="Environment=PYTHONPATH=$worktree/src" \
    "$python" -m bithumb_coin_trader.runtime_observer \
      --data-dir "/var/lib/bitcoin-trader/6h-validation/aws-validation-observability-6h-20260918-20260918T175000Z-v2" \
      --epoch "aws-validation-observability-6h-20260918-20260918T175000Z-v2" \
      --run-id "aws-validation-observability-6h-run-20260918T175000Z-v2" \
      --unit-name "bitcoin-trader-6h-aws-validation-observability-6h-run-20260918T175000Z-v2.service" \
      --poll-interval 15.0 \
      --s3-publish-interval 60.0 \
      --stale-threshold 30.0 \
      --s3-bucket "bitcoin-trader-aws-apne2-research-ap-northeast-2-080109295433" \
      --s3-prefix "market-data/temporary/aws-validation-observability-6h-20260918-20260918T175000Z-v2" \
      --allow-s3-write
  ...
  if ! "$python" -m bithumb_coin_trader.observer_readiness \
      --health-dir "/var/lib/bitcoin-trader/6h-validation/aws-validation-observability-6h-20260918-20260918T175000Z-v2/health" \
      --epoch "aws-validation-observability-6h-20260918-20260918T175000Z-v2" \
      --run-id "aws-validation-observability-6h-run-20260918T175000Z-v2" \
      --unit-name "$obs_unit" \
      --max-age 30.0 \
