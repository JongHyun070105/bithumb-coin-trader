...
artifacts_dir="/var/lib/bitcoin-trader/launch-artifacts/aws-validation-observability-30h-20260919-20260919T095000Z-v2"
...
if [ "$is_launch" = true ]; then
  ...
  planned_start="2026-09-19T09:50:00+00:00"
  qual_start="2026-09-19T10:00:00+00:00"
  ...
  obs_unit="bitcoin-trader-obs-aws-validation-observability-30h-run-20260919T095000Z-v2.service"
  echo "[LAUNCH] Step B: Starting runtime observer unit $obs_unit (pinned to user: bitcoin-trader)..."
  systemd-run \
    --unit="$obs_unit" \
    --description="Runtime Observer for aws-validation-observability-30h-run-20260919T095000Z-v2" \
    --service-type=simple \
    --no-block \
    --uid=bitcoin-trader \
    ...
    "$python" -m bithumb_coin_trader.runtime_observer \
      ...
      --epoch "aws-validation-observability-30h-20260919-20260919T095000Z-v2" \
      --run-id "aws-validation-observability-30h-run-20260919T095000Z-v2" \
      --unit-name "bitcoin-trader-transient-aws-validation-observability-30h-run-20260919T095000Z-v2.service" \
      --poll-interval 15.0 \
      --s3-publish-interval 60.0 \
      --stale-threshold 30.0 \
      --s3-bucket "bitcoin-trader-aws-apne2-research-ap-northeast-2-080109295433" \
      --s3-prefix "market-data/temporary/aws-validation-observability-30h-20260919-20260919T095000Z-v2" \
      --allow-s3-write
  ...
  if ! "$python" -m bithumb_coin_trader.observer_readiness \
      --health-dir "/var/lib/bitcoin-trader/30h-validation/aws-validation-observability-30h-20260919-20260919T095000Z-v2/health" \
      --epoch "aws-validation-observability-30h-20260919-20260919T095000Z-v2" \
      --run-id "aws-validation-observability-30h-run-20260919T095000Z-v2" \
      --unit-name "$obs_unit" \
      --max-age 30.0 \
