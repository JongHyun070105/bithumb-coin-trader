...
artifacts_dir="/var/lib/bitcoin-trader/launch-artifacts/aws-validation-observability-90m-20260918-20260918T005000Z-v10"
...
if [ "$is_launch" = true ]; then
  ...
  obs_unit="bitcoin-trader-obs-aws-validation-observability-90m-run-20260918T005000Z-v10.service"
  echo "[LAUNCH] Step B: Starting runtime observer unit $obs_unit (pinned to user: bitcoin-trader)..."
  systemd-run \
    --unit="$obs_unit" \
    --description="Runtime Observer for aws-validation-observability-90m-run-20260918T005000Z-v10" \
    --service-type=simple \
    --no-block \
    --uid=bitcoin-trader \
    --property="Environment=PYTHONPATH=$worktree/src" \
    "$python" -m bithumb_coin_trader.runtime_observer \
      --data-dir "/var/lib/bitcoin-trader/90m-validation/aws-validation-observability-90m-20260918-20260918T005000Z-v10" \
      --epoch "aws-validation-observability-90m-20260918-20260918T005000Z-v10" \
      --run-id "aws-validation-observability-90m-run-20260918T005000Z-v10" \
      --unit-name "bitcoin-trader-90m-aws-validation-observability-90m-run-20260918T005000Z-v10.service" \
      --poll-interval 15.0 \
      --s3-publish-interval 60.0 \
      --stale-threshold 30.0 \
      --s3-bucket "bitcoin-trader-aws-apne2-research-ap-northeast-2-080109295433" \
      --s3-prefix "market-data/temporary/aws-validation-observability-90m-20260918-20260918T005000Z-v10" \
      --allow-s3-write
  ...
  if ! "$python" -m bithumb_coin_trader.observer_readiness \
      --health-dir "/var/lib/bitcoin-trader/90m-validation/aws-validation-observability-90m-20260918-20260918T005000Z-v10/health" \
      --epoch "aws-validation-observability-90m-20260918-20260918T005000Z-v10" \
      --run-id "aws-validation-observability-90m-run-20260918T005000Z-v10" \
      --unit-name "$obs_unit" \
      --max-age 30.0 \
