...
worktree="/var/lib/bitcoin-trader/runtime-worktrees/aws-6h-v4-525a7d3"
python="/var/lib/bitcoin-trader/venv-pre-soak/bin/python"
artifacts_dir="/var/lib/bitcoin-trader/launch-artifacts/aws-validation-observability-6h-20260921-20260921T095000Z-v4"
...
if [ "$is_launch" = true ]; then
  ...
  planned_start="2026-09-21T09:50:00+00:00"
  qual_start="2026-09-21T10:00:00+00:00"
  ...
  obs_unit="bitcoin-trader-obs-aws-validation-observability-6h-run-20260921T095000Z-v4.service"
  echo "[LAUNCH] Step B: Starting runtime observer unit $obs_unit (pinned to user: bitcoin-trader)..."
  systemd-run \
    --unit="$obs_unit" \
    --description="Runtime Observer for aws-validation-observability-6h-run-20260921T095000Z-v4" \
    --service-type=simple \
    --no-block \
    --uid=bitcoin-trader \
    --property="Environment=PYTHONPATH=$worktree/src" \
    "$python" -m bithumb_coin_trader.runtime_observer \
      --data-dir "/var/lib/bitcoin-trader/6h-validation/aws-validation-observability-6h-20260921-20260921T095000Z-v4" \
      --epoch "aws-validation-observability-6h-20260921-20260921T095000Z-v4" \
      --run-id "aws-validation-observability-6h-run-20260921T095000Z-v4" \
      --unit-name "bitcoin-trader-6h-aws-validation-observability-6h-run-20260921T095000Z-v4.service" \
      --poll-interval 15.0 \
      --s3-publish-interval 60.0 \
      --stale-threshold 30.0 \
      --s3-bucket "bitcoin-trader-aws-apne2-research-ap-northeast-2-080109295433" \
      --s3-prefix "market-data/temporary/aws-validation-observability-6h-20260921-20260921T095000Z-v4" \
      --allow-s3-write
  ...
  if ! "$python" -m bithumb_coin_trader.observer_readiness \
      --health-dir "/var/lib/bitcoin-trader/6h-validation/aws-validation-observability-6h-20260921-20260921T095000Z-v4/health" \
      --epoch "aws-validation-observability-6h-20260921-20260921T095000Z-v4" \
      --run-id "aws-validation-observability-6h-run-20260921T095000Z-v4" \
      --unit-name "$obs_unit" \
      --max-age 30.0 \
