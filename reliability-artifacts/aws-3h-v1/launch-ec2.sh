...
artifacts_dir="/var/lib/bitcoin-trader/launch-artifacts/aws-validation-observability-3h-20260918-20260918T055000Z-v1"
...
if [ "$is_launch" = true ]; then
  ...
  planned_start="2026-09-18T05:50:00+00:00"
  qual_start="2026-09-18T06:00:00+00:00"
  ...
  obs_unit="bitcoin-trader-obs-aws-validation-observability-3h-run-20260918T055000Z-v1.service"
  echo "[LAUNCH] Step B: Starting runtime observer unit $obs_unit (pinned to user: bitcoin-trader)..."
  systemd-run \
    --unit="$obs_unit" \
    --description="Runtime Observer for aws-validation-observability-3h-run-20260918T055000Z-v1" \
    --service-type=simple \
    --no-block \
    --uid=bitcoin-trader \
    --property="Environment=PYTHONPATH=$worktree/src" \
    "$python" -m bithumb_coin_trader.runtime_observer \
      --data-dir "/var/lib/bitcoin-trader/3h-validation/aws-validation-observability-3h-20260918-20260918T055000Z-v1" \
      --epoch "aws-validation-observability-3h-20260918-20260918T055000Z-v1" \
      --run-id "aws-validation-observability-3h-run-20260918T055000Z-v1" \
      --unit-name "bitcoin-trader-3h-aws-validation-observability-3h-run-20260918T055000Z-v1.service" \
      --poll-interval 15.0 \
      --s3-publish-interval 60.0 \
      --stale-threshold 30.0 \
      --s3-bucket "bitcoin-trader-aws-apne2-research-ap-northeast-2-080109295433" \
      --s3-prefix "market-data/temporary/aws-validation-observability-3h-20260918-20260918T055000Z-v1" \
      --allow-s3-write
  ...
  if ! "$python" -m bithumb_coin_trader.observer_readiness \
      --health-dir "/var/lib/bitcoin-trader/3h-validation/aws-validation-observability-3h-20260918-20260918T055000Z-v1/health" \
      --epoch "aws-validation-observability-3h-20260918-20260918T055000Z-v1" \
      --run-id "aws-validation-observability-3h-run-20260918T055000Z-v1" \
      --unit-name "$obs_unit" \
      --max-age 30.0 \
...
exec "$python" "$worktree/scripts/launch_short_smoke_transient.py" \
  ...
  --collection-duration-seconds 10800 \
  --finalization-timeout-seconds 180 \
  --supervisor-hard-ceiling-seconds 10980 \
  --systemd-runtime-max-seconds 11040 \
