export async function calculateSha256(
  ...
  // Pure deterministic software fallback for test/node environments without crypto.subtle
  return simpleSha256Fallback(new Uint8Array(buffer))
}

// Minimal SHA-256 standard fallback
export function simpleSha256Fallback(bytes: Uint8Array): string {
  const K = [
...
export const sha256Text = (text: string): string =>
  simpleSha256Fallback(new TextEncoder().encode(text))
