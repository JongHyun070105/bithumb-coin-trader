...
export const EvidenceArtifactCard: React.FC<EvidenceArtifactCardProps> = ({
  artifact,
  onRemove,
}) => {
  const isOk =
    artifact.parseStatus === 'SUCCESS' &&
    artifact.validationLevel !== 'RECOGNIZED_INVALID'
  const isUnknown = artifact.parseStatus === 'UNKNOWN_TYPE'
  const isFailed =
    artifact.parseStatus === 'PARSE_FAILED' ||
    artifact.validationLevel === 'RECOGNIZED_INVALID'
  ...
  return (
    <div
      className={`artifact-card status-${artifact.parseStatus.toLowerCase()}`}
    >
      <div className="card-top">
        <div className="card-identity">
          <FileJson size={16} className="file-icon" />
          <strong className="file-name" title={artifact.fileName}>
            {artifact.fileName}
          </strong>
        </div>
        {onRemove && (
      ...
      <div className="card-meta">
        <span className={`type-tag type-${artifact.type}`}>{label}</span>
      ...
      <div className="card-hash">
        <span>FILE BYTE SHA256</span>
        <Hash size={12} className="hash-icon" />
        <code className="hash-text" title={artifact.calculatedSha256}>
          {artifact.calculatedSha256.slice(0, 16)}...
          {artifact.calculatedSha256.slice(-8)}
        </code>
      </div>
      ...
      <div className="card-footer">
        {isOk && (
          <span className="status-indicator status-ok">
            ...
            <span>{artifact.validationLevel ?? 'PARSED — NOT VERIFIED'}</span>
          </span>
        )}
        {isUnknown && (
